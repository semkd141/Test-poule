...
const boolishLegacy = z.preprocess((v) => {
    if (v === undefined || v === "")
        return true;
    if (typeof v === "boolean")
        return v;
...
const envSchema = z.object({
     ...
     */
    SUPABASE_SERVICE_ROLE_KEY: z.preprocess((v) => (v === "" || v === undefined ? undefined : v), z.string().min(20).optional()),
    /** Dashboard → API → JWT secret (HS256 access tokens from Supabase Auth). */
    SUPABASE_JWT_SECRET: z.string().min(1, "SUPABASE_JWT_SECRET is required"),
    API_FOOTBALL_KEY: z.preprocess((v) => (v === "" || v === undefined ? undefined : v), z.string().min(8).optional()),
    RESEND_API_KEY: z.preprocess((v) => (v === "" || v === undefined ? undefined : v), z.string().min(8).optional()),
    RESEND_FROM_EMAIL: z.preprocess((v) => (v === "" || v === undefined ? undefined : v), z.string().email().optional()),
    ...
    BREVO_API_KEY: z.preprocess((v) => (v === "" || v === undefined ? undefined : v), z.string().min(8).optional()),
    /** Prefer REST sends (`xkeysib-…`, SMTP & API → API keys). Use when `BREVO_API_KEY` is only an SMTP password or SMTP login fails. */
    BREVO_REST_API_KEY: z.preprocess((v) => (v === "" || v === undefined ? undefined : v), z.string().min(8).optional()),
    BREVO_FROM_EMAIL: z.preprocess((v) => (v === "" || v === undefined ? undefined : v), z.string().email().optional()),
    BREVO_FROM_NAME: z.preprocess((v) => (v === "" || v === undefined ? undefined : v), z.string().min(1).optional()),
    /** SMTP relay auth.user (Brevo: SMTP & API → SMTP). Often `*@brevo.com`; defaults to `BREVO_FROM_EMAIL` only if your login matches From. */
    BREVO_SMTP_LOGIN: z.preprocess((v) => (v === "" || v === undefined ? undefined : v), z.string().email().optional()),
    CRON_SECRET: z.preprocess((v) => (v === "" || v === undefined ? undefined : v), z.string().min(8).optional()),
    PARTICIPANT_LEGACY_OPEN_MUTATIONS: boolishLegacy,
    ADMIN_API_SECRET: z.preprocess((v) => (v === "" || v === undefined ? undefined : v), z.string().min(8).optional()),
    /** Optional fixed admin user id (Supabase auth.users.id) with full backend admin authority. */
    ADMIN_UID: z.preprocess((v) => (v === "" || v === undefined ? undefined : v), z.string().uuid().optional()),
    /** Public web app origin for invitation links, e.g. https://app.example.com (no trailing slash). */
    PUBLIC_APP_URL: z.preprocess((v) => (v === "" || v === undefined ? undefined : v), z.string().url().optional()),
    LOG_LEVEL: z
...
export function loadEnv() {
    ...
    if (!parsed.success) {
        ...
        throw new Error("Invalid environment configuration. Check SUPABASE_URL, SUPABASE_KEY, SUPABASE_JWT_SECRET, and PORT.");
    }
    cached = parsed.data;
    return parsed.data;
}
