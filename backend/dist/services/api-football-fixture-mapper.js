...
export function roundBucket(round) {
    const r = String(round ?? "").toLowerCase();
    if (!r)
        return "unknown";
    if (r.includes("group stage") || /^group\b/.test(r))
        return "group";
    if (r.includes("round of 16") ||
        r.includes("16th finals") ||
        /\bl\s*\/\s*16\b/.test(r))
        return "r16";
    if (r.includes("quarter") || /\bqf\b/.test(r))
        return "qr";
    if (r.includes("semi"))
        return "sf";
    if (r.includes("3rd place") || r.includes("third place"))
        return "tp";
    if (/^final\b/.test(r) || r.trim() === "final")
        return "final";
    return "unknown";
}
export function stageForBucket(bucket) {
    switch (bucket) {
        case "group":
            return "group";
        case "r16":
...
export function kickoffUnix(item) {
...
export function extractFixtureRow(item, apiFootballLeagueId, season) {
    const fixture = item?.fixture ?? {};
    const league = item?.league ?? {};
    const teams = item?.teams ?? {};
    const home = teams?.home?.name ?? null;
    const away = teams?.away?.name ?? null;
    const location = fixture?.venue?.name !== undefined && fixture?.venue?.name !== null
        ? String(fixture.venue.name)
        : null;
    const apiId = fixture.id;
    if (typeof apiId !== "number" || !Number.isFinite(apiId) || apiId <= 0)
        return null;
    const bucket = roundBucket(league.round);
    const dateRaw = fixture.date;
    const kickoff_at = typeof dateRaw === "string" && dateRaw.trim() ? dateRaw.trim() : null;
    return {
        _bucket: bucket,
...
export function assignLocalKeys(rows) {
    ...
    return rows.map((row) => {
        ...
        if (!lk)
            throw new Error("assignLocalKeys: missing local_key");
        return {
...
export function mapApiResponseToFixtureRows(items, apiFootballLeagueId, season) {
    const internal = [];
...
function throwIfNonEmptyErrors(data) {
    ...
    if (hasErr)
        throw new Error(`API-Football: ${JSON.stringify(err)}`);
}
/**
 ...
 */
export async function fetchAllFixturesFromApiFootball(params) {
    const { apiKey, league, season, timeoutMs = 120_000 } = params;
    const key = typeof apiKey === "string" ? apiKey.trim() : "";
    if (!key)
        throw new Error("API-Football key is empty");
    ...
    const d0 = first.data;
    const chunk0 = Array.isArray(d0?.response) ? d0.response : [];
    if (chunk0.length === 0) {
        throwIfNonEmptyErrors(d0);
        return [];
    }
    const totalRaw = d0?.paging?.total;
    const totalPages = typeof totalRaw === "number" && Number.isFinite(totalRaw) && totalRaw > 0
        ? Math.floor(totalRaw)
        : 1;
    if (totalPages <= 1) {
        return chunk0;
    }
    const extraPages = await Promise.all(Array.from({ length: totalPages - 1 }, (_, i) => client.get("/fixtures", {
    ...
    for (const res of extraPages) {
        ...
        const chunk = Array.isArray(d?.response) ? d.response : [];
        all.push(...chunk);
    }
    return all;
}
//# sourceMappingURL=api-football-fixture-mapper.js.map