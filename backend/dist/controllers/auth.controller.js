...
const signUpBodySchema = z.object({
    ...
    redirect_to: z.preprocess((v) => (v === "" || v === undefined ? undefined : v), z.string().url("redirect_to must be a valid URL").optional()),
});
...
function rowsFromParticipantLookup(data) {
    if (!Array.isArray(data))
        return [];
    return data.filter((x) => Boolean(x && typeof x === "object" && !Array.isArray(x)));
}
const accountAlreadyExistsMessage = (email) => `A user account already exists for "${email}". Please log in instead of signing up again.`;
const noTeamRegistrationMessage = (email) => `No team registration exists for "${email}". Register your team on the Register tab first using this exact email address, then return here to create your login password.`;
function userIdFromSignupBody(data) {
...
function isSupabaseAuthUserAlreadyExists(payload) {
...
function isAdminRequest(req, env) {
    if (env.ADMIN_API_SECRET && req.get("x-admin-secret") === env.ADMIN_API_SECRET)
        return true;
    if (env.ADMIN_UID && String(req.supabaseUser?.sub ?? "") === env.ADMIN_UID)
        return true;
...
export function createAuthHandlers(gateway, env) {
    ...
    return {
        ...
        signInWithPassword: asyncHandler(async (req, res) => {
        ...
        signUpWithPassword: asyncHandler(async (req, res) => {
            ...
            if (existing.length === 0) {
                ...
                try {
                    ...
                    if (uid)
                        await gateway.adminDeleteUser(uid);
        ...
        sendInvite: asyncHandler(async (req, res) => {
            ...
            const out = await mailer.sendCompetitionInvite(parsed.data.email.trim(), parsed.data.competitionName.trim(), parsed.data.inviteUrl.trim());
