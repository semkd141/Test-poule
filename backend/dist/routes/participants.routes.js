...
export function createParticipantsRouter(gateway, env, log) {
    const router = Router();
    const handlers = createParticipantsHandlers(gateway, env, log);
    if (typeof handlers.joinCompetition !== "function") {
        throw new Error("createParticipantsHandlers must export joinCompetition — the API cannot register POST /participants/join.");
    }
    ...
    router.get("/participants/registerable-competitions", requireJwt, handlers.listMyRegisterableCompetitions);
