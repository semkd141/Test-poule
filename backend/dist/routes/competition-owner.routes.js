...
import { fetchAllFixturesFromApiFootball, mapApiResponseToFixtureRows, } from "../services/api-football-fixture-mapper.js";
...
const competitionPutSchema = z.object({
    ...
    name: z.string().min(2),
    league_type: z.string().min(1, "league_type is required"),
    season_label: z.preprocess((v) => (v === "" || v === undefined ? null : v), z.union([z.string(), z.null()])),
    starts_at: z.preprocess((v) => (v === "" || v === undefined ? null : v), z.union([z.string().datetime(), z.null()])),
...
function resolveApiFootballLeagueSeason(body, meta, competitionRow) {
    ...
    throw new HttpError(400, "Set api_football.season in pool metadata, pass season in the import body, or pass both league and season. The pool’s league type sets the default API-Football league id.");
}
...
async function requesterEmail(req, gateway) {
...
export function createCompetitionOwnerRouter(gateway, env, log) {
    const router = Router();
    ...
    router.get("/:competitionId/fixture-mappings", asyncHandler(async (req, res) => {
        const id = String(req.params.competitionId ?? "").trim();
        if (!id)
            throw new HttpError(400, "competition id required");
        const row = await gateway.getCompetitionById(id);
        if (!row)
            throw new HttpError(404, "Competition not found");
        if (!canManageCompetition(req, env, row))
            throw new HttpError(403, "Forbidden");
        const competitionId = Number(row.id);
        if (!Number.isFinite(competitionId))
            throw new HttpError(500, "Invalid competition id");
        const out = await gateway.listFixtureMappings(competitionId);
        res.json(Array.isArray(out) ? out : []);
    }));
    router.post("/:competitionId/import-api-football-fixtures", asyncHandler(async (req, res) => {
        const id = String(req.params.competitionId ?? "").trim();
        if (!id)
            throw new HttpError(400, "competition id required");
        const row = await gateway.getCompetitionById(id);
        ...
        if (already) {
            ...
            res.json({
                ...
                league,
                season,
                message: "Fixture mappings already exist for this API-Football league and season (shared across pools). Metadata updated; no API call.",
            });
            return;
        }
        if (!env.API_FOOTBALL_KEY || env.API_FOOTBALL_KEY.length < 8) {
            throw new HttpError(503, "No other pool has this league/season with fixtures yet, and API_FOOTBALL_KEY is not configured. Add a pool with imported fixtures or set API_FOOTBALL_KEY.");
        }
        let items;
        try {
    ...
    router.post("/:competitionId/fetch-fixture-squad", asyncHandler(async (req, res) => {
        ...
        if (!comp)
            throw new HttpError(404, "Competition not found");
        if (!canManageCompetition(req, env, comp))
            throw new HttpError(403, "Forbidden");
        ...
        const match = maps.find((m) => {
            if (!m || typeof m !== "object" || Array.isArray(m))
                return false;
            return Number(m.api_fixture_id) === fixtureId;
        });
        if (!match || typeof match !== "object" || Array.isArray(match)) {
            throw new HttpError(400, "This fixture id is not linked in this pool’s fixture mappings (set the API fixture id on the row first).");
        }
        const mapRec = match;
        const mapLeague = Number(mapRec.api_football_league_id);
        const mapSeason = Number(mapRec.season);
        const leagueSeasonFromMapping = Number.isFinite(mapLeague) && mapLeague > 0 && Number.isFinite(mapSeason) && mapSeason > 0
    ...
    router.post("/:competitionId/fetch-all-fixture-squads", asyncHandler(async (req, res) => {
        const cid = String(req.params.competitionId ?? "").trim();
        if (!cid)
            throw new HttpError(400, "competition id required");
        const comp = await gateway.getCompetitionById(cid);
        ...
        for (const m of maps) {
            if (!m || typeof m !== "object" || Array.isArray(m))
                continue;
            const fid = Math.floor(Number(m.api_fixture_id));
            if (!Number.isFinite(fid) || fid <= 0 || seen.has(fid))
                continue;
            seen.add(fid);
            fixtureIds.push(fid);
        }
        ...
        startFixtureSquadBackgroundBatch({ fixtureIds, leagueId: scope.leagueId, season: scope.season }, gateway, env, log);
        res.json({ message: FIXTURE_SQUAD_BATCH_BACKGROUND_MESSAGE, queued: fixtureIds.length });
    }));
    router.patch("/:competitionId/fixture-mappings/:mappingId", asyncHandler(async (req, res) => {
        const cid = String(req.params.competitionId ?? "").trim();
        const mid = String(req.params.mappingId ?? "").trim();
        if (!cid || !mid)
            throw new HttpError(400, "competition id and mapping id required");
        const comp = await gateway.getCompetitionById(cid);
        if (!comp)
            throw new HttpError(404, "Competition not found");
        if (!canManageCompetition(req, env, comp))
        ...
        if (!mapRow)
            throw new HttpError(404, "Fixture mapping not found");
        const scope = gateway.getFixtureMappingScopeForCompetition(comp);
        if (!scope)
            throw new HttpError(400, "Pool has no API-Football league/season for fixture mappings");
        if (Number(mapRow.api_football_league_id) !== scope.leagueId ||
    ...
    router.get("/:competitionId/participants", asyncHandler(async (req, res) => {
        const id = String(req.params.competitionId ?? "").trim();
        if (!id)
            throw new HttpError(400, "competition id required");
        const row = await gateway.getCompetitionById(id);
    ...
    router.get("/:competitionId/invites", asyncHandler(async (req, res) => {
        const id = String(req.params.competitionId ?? "").trim();
        if (!id)
            throw new HttpError(400, "competition id required");
        const row = await gateway.getCompetitionById(id);
    ...
    router.post("/:competitionId/invites", asyncHandler(async (req, res) => {
        const id = String(req.params.competitionId ?? "").trim();
        ...
        if (!ownerEmail)
            throw new HttpError(400, "Your account email is required to send invitations");
        ...
        res.status(201).json({
            ok: true,
            id: insRow && typeof insRow === "object" && insRow !== null && "id" in insRow ? insRow.id : undefined,
            email: inviteEmail,
            expires_at: expires.toISOString(),
            emailed: send.sent,
    ...
    router.patch("/:id", asyncHandler(async (req, res) => {
        ...
        if (!existing)
            throw new HttpError(404, "Competition not found");
        if (!canManageCompetition(req, env, existing))
            throw new HttpError(403, "Forbidden");
        ...
        let body = bodyRaw.slug !== undefined ? { ...bodyRaw, slug: bodyRaw.slug.trim().toLowerCase() } : { ...bodyRaw };
...
//# sourceMappingURL=competition-owner.routes.js.map