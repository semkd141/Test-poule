...
const brevoSmtpNoExplicitLogin = brevoKey.startsWith("xsmtpsib-") &&
    !env.BREVO_REST_API_KEY?.trim() &&
    !env.BREVO_SMTP_LOGIN?.trim();
if (brevoSmtpNoExplicitLogin) {
    logger.warn("Brevo SMTP: BREVO_SMTP_LOGIN is unset — SMTP auth must use the login from Brevo → SMTP & API → SMTP (not your From address). Set BREVO_SMTP_LOGIN or add BREVO_REST_API_KEY (xkeysib-) to use the REST API instead.");
}
